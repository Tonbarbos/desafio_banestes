...
import type { Cliente, Conta } from './utils/dataFetcher';
import { fetchClientes, fetchContas } from './utils/dataFetcher';
import ClientDetail from './components/ClientDetail';
import Reports from './components/Reports';
import './App.css';

const ITEMS_PER_PAGE_OPTIONS = [10, 16, 32, 64];
...
function App() {
  ...
  const [filterByAgency, setFilterByAgency] = useState<boolean>(false);
  ...
  const filteredAndSortedClientes = useMemo(() => {
    ...
    if (searchTerm) {
      currentClientes = currentClientes.filter(cliente => {
        ...
        const clientContas = contas.filter(c => c.cpfCnpjCliente === cliente.cpfCnpj);
        const matchesAgency = filterByAgency && clientContas.some(conta => 
            conta.nomeAgencia?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            conta.enderecoAgencia?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            String(conta.codigoAgencia).includes(searchTerm)
        );
